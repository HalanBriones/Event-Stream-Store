...
import { ArrowLeft, BookOpen, Users, CheckCircle2, Clock, GraduationCap, Activity, ChevronRight, BarChart3 } from "lucide-react";
import { format } from "date-fns";

const paceConfig = {
  Rushing: { color: "text-destructive", bg: "bg-destructive/10", badge: "bg-destructive/10 text-destructive border-destructive/20" },
  Engaged: { color: "text-blue-600",    bg: "bg-blue-500/10",    badge: "bg-blue-500/10 text-blue-600 border-blue-500/20" },
  Steady:  { color: "text-green-600",   bg: "bg-green-500/10",   badge: "bg-green-500/10 text-green-700 border-green-500/20" },
};
          ...
          <div className="flex items-center gap-4">
            ...
            <div>
              <div className="flex items-center gap-2 mb-1">
                <span className="px-2 py-0.5 rounded-full bg-orange-500/10 text-orange-600 text-[10px] font-bold uppercase tracking-wider">Course Insights</span>
              ...
              <div className="mt-2 h-1.5 w-full bg-muted rounded-full overflow-hidden">
                <div
                  ...
                  style={{
                    ...
                    background: (stats?.completionRate ?? 0) >= 70 ? '#22c55e' : (stats?.completionRate ?? 0) >= 40 ? '#f97316' : '#ef4444'
                  }}
                />
              </div>
            </CardContent>
              ...
              {[
                ...
                <div key={label} className={`rounded-2xl ${bg} p-4 text-center`}>
                  ...
                  <div className="text-[10px] text-muted-foreground mt-0.5">
                    {stats?.totalEnrolled > 0 ? Math.round((count / stats.totalEnrolled) * 100) : 0}% of students
                  </div>
                </div>
              ))}
            </div>
        ...
        <div className="space-y-4">
          ...
          <Accordion type="single" collapsible className="space-y-3">
            {stats?.lessons?.map((lesson: any) => (
              <AccordionItem key={lesson.lessonId} value={`lesson-${lesson.lessonId}`} className="border-none rounded-2xl px-6 bg-background shadow-sm overflow-hidden">
                  ...
                  <div className="flex items-center justify-between w-full pr-4">
                    <div className="flex items-center gap-3">
                      ...
                      <div className="text-left">
                        ...
                        <div className="text-xs text-muted-foreground">Avg time: {formatDuration(lesson.avgDurationMinutes)}</div>
                      </div>
                    </div>
                          ...
                          <div className="flex-1 h-1.5 bg-muted rounded-full overflow-hidden">
                            <div
                              ...
                              style={{
                                ...
                                background: lesson.completionRate >= 70 ? '#22c55e' : lesson.completionRate >= 40 ? '#f97316' : '#ef4444'
                              }}
                            />
                          </div>
                ...
                <AccordionContent className="pb-6">
                  {lesson.quizzes?.length > 0 ? (
                    <div className="space-y-3 pt-2">
                      <div className="text-[10px] font-black uppercase tracking-widest text-muted-foreground/60 px-1">Quizzes</div>
                      {lesson.quizzes.map((quiz: any) => (
                        <div key={quiz.quizId} className="bg-muted/30 rounded-xl p-4 flex flex-col md:flex-row md:items-center justify-between gap-3">
                          <div>
                            <div className="font-bold text-sm">Quiz {quiz.quizId}</div>
                            <div className="text-xs text-muted-foreground">Avg completion time: {formatDuration(quiz.avgDurationMinutes)}</div>
                          </div>
                          <div className="flex items-center gap-6">
                            ...
                            <div className="min-w-[80px]">
                              <div className="text-[10px] font-bold uppercase text-muted-foreground mb-1">Submit Rate</div>
                              <div className="flex items-center gap-1.5">
                                <div className="flex-1 h-1.5 bg-muted rounded-full overflow-hidden">
                                  <div
                                    className="h-full rounded-full"
                                    style={{
                                      ...
                                      background: quiz.submissionRate >= 70 ? '#22c55e' : quiz.submissionRate >= 40 ? '#f97316' : '#ef4444'
                                    }}
                                  />
                                </div>
                    ...
                    <p className="text-xs text-muted-foreground/60 pt-2">No quizzes recorded for this lesson.</p>
                  )}
                </AccordionContent>
              </AccordionItem>
            ))}
